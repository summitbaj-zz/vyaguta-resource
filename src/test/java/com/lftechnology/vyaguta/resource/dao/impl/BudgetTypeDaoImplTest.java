...
public class BudgetTypeDaoImplTest {
  ...
  public void testSaveBudgetTypeAndExpectDataAccessExceptionWhenEntityManagerFails() {
    ...
    Mockito
        .doThrow(
            new PersistenceException("Exception while persisting budget type"))
        .when(em).persist(budgetType);
  ...
  public void testFindBudgetTypeById() {
    // arrange
    BudgetType budgetType = this.buildBudgetType();
    Mockito.when(em.find(Matchers.anyObject(), Matchers.anyString()))
    ...
    // assert
    Mockito.verify(em, Mockito.times(1)).find(Matchers.anyObject(),
        Matchers.anyString());
  }
  ...
  public void testFindBudgetType() {
    ...
    // assert
    Mockito.verify(this.criteriaUtil, Mockito.times(1)).find(BudgetType.class,
        1, 20);
