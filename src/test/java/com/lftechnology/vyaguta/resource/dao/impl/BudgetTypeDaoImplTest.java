...
public class BudgetTypeDaoImplTest {
	...
	public void testSaveBudgetTypeAndExpectDataAccessExceptionWhenEntityManagerFails() {
		...
		BudgetType budgetType = this.buildBudgetType();
		Mockito.doThrow(
			new PersistenceException("Exception while persisting budget type")).when(
	...
	public void testFindBudgetTypeById() {
		...
		// arrange
		BudgetType budgetType = this.buildBudgetType();
		Mockito.when(em.find(Matchers.anyObject(), Matchers.anyString())).thenReturn(
			budgetType);
		...
		// assert
		Mockito.verify(em, Mockito.times(1)).find(
	...
	public void testFindAllBudgetType() {
		...
		budgetTypes.add(new BudgetType());
		Mockito.when(this.criteriaUtil.findAll(BudgetType.class)).thenReturn(
			budgetTypes);
		...
		// assert
		Mockito.verify(this.criteriaUtil, Mockito.times(1)).findAll(
			BudgetType.class);
	...
	public void testCountBudgetType() {
		...
		// assert
		Mockito.verify(this.criteriaUtil, Mockito.times(1)).count(
			BudgetType.class);
	...
	public void testFindBudgetType() {
		...
		budgetTypes.add(new BudgetType());
		budgetTypes.add(new BudgetType());
		Mockito.when(this.criteriaUtil.find(BudgetType.class, 1, 20)).thenReturn(
			budgetTypes);
		...
		// assert
		Mockito.verify(this.criteriaUtil, Mockito.times(1)).find(
			BudgetType.class, 1, 20);
