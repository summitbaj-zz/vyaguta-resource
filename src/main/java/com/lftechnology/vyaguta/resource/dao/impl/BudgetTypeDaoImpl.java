...
public class BudgetTypeDaoImpl extends BaseDao<BudgetType, String>implements BudgetTypeDao {
    ...
    @Override
    protected List<Order> getSortOrder(List<String> sorts, CriteriaBuilder criteriaBuilder, Root<BudgetType> root) {
        List<Order> orders = new ArrayList<>();
        for (String sort : sorts) {
            String sortField = "-".equals(sort.substring(0, 1)) ? sort.replaceFirst("-", "") : sort;
            if (sortField.equals(CommonConstant.TITLE)) {
                orders.add(
                        ("-").equals(sort.substring(0, 1)) ? criteriaBuilder.desc(root.get(sort.replaceFirst("-", "")))
